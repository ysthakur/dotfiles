...
let dark_theme = {
    ...
    range: white
    float: white
    string: {|| if $in =~ '^#[a-fA-F\d]{6}$' { $in } else { 'dark_gray' } }
    nothing: white
    binary: white
    cell-path: white
...
$env.config = {
    ...
    color_config: (if $env.LIGHT_THEME {$light_theme} else {$dark_theme}) # if you want a more interesting theme, you can replace the empty record with `$dark_theme`, `$light_theme` or another custom record
    edit_mode: emacs # emacs, vi
    render_right_prompt_on_last_line: false # true or false to enable or disable right prompt to be rendered on last line of the prompt.
    highlight_resolved_externals: true # enables highlighting of external commands in the repl resolved by which
